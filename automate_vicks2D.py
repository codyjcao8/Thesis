# -*- coding: utf-8 -*-
"""
Created on Sat Mar 10 00:51:05 2018
...
#        plt.pause(2)
        ...
        while 1:
            ...
            print(time  , picture[middle+radius-7], picture[middle+radius+7] )
            if picture[middle + radius - 7] < 0.5:
                #   record that strategy 2 won
                data.append([eps, alpha, d, pop, mu, mu2, mu/mu2, 2, time])
                lo = alpha
                break
                
            if picture[middle + radius + 7] > 0.5:
                #   record that strategy 1 won
                data.append([eps, alpha, d, pop, mu, mu2, mu/mu2, 1, time])
                hi = alpha
                break
            
            if time == 80:
                #   something                  
                data.append([eps, alpha, d, pop, mu, mu2, mu/mu2, 3, time])
                nextmu = True
                break 
...
cwd = os.getcwd()
filename = "vickers2D_" +str(radius) + ".txt"
