# -*- coding: utf-8 -*-
"""
Created on Sun Jan 21 23:09:50 2018
...
numpy.random.seed(1287)
#numpy.random.seed(12457)

mod = SourceModule("""
#include <math.h>

__global__ void fitness(double *x, double *y, double *z, double eps)
{
  ...
  x[i] = tempx + tempx*eps*(   (tempx*a1 + tempy*b1 + tempz*c1)/pop  -  avgfit/(pop*pop)  );
  y[i] = tempy + tempy*eps*(   (tempx*a2 + tempy*b2 + tempz*c2)/pop  -  avgfit/(pop*pop)  );
  z[i] = tempz + tempz*eps*(   (tempx*a3 + tempy*b3 + tempz*c3)/pop  -  avgfit/(pop*pop)  ); 
}
...
__global__ void display(double *picture, double *destx, double *desty)
{
  const int i = threadIdx.x + blockDim.x * blockIdx.x;
...
display = mod.get_function("display")
