# -*- coding: utf-8 -*-
"""
Created on Fri Mar  2 01:27:29 2018
...
__global__ void fitness(double *x, double *y, double eps, double a, double b, double c, double d)
{
  ...
  double lambda1 = 1;
  double lambda2 = .02;
  double backfit = lambda1 - lambda2*pop;
  
  y[i] = tempy + tempy*eps*(   (tempx*c + tempy*d)/(tempx+tempy)  +  backfit );
  x[i] = tempx + tempx*eps*(   (tempx*a + tempy*b)/(tempx+tempy)  +  backfit );
...
# parameters to test
mulist  = numpy.linspace(4.5, 5.7, 15)
        ...
        while 1:
            ...
            for count in range(1000):    
                ...
                diffuse_display(
                        ...
                        numpy.float64(eps),numpy.int64(side), block = (block,1,1), grid=(grid,1))
            ...
            if picture[middle + 5] < 0.5:
                ...
                data.append([eps, alpha, d, pop, init_pop2, mu, mu2, 3, time])
                lo = alpha
                break
...
cwd = os.getcwd()
filename = "data_test_cressman_"+str(pop)+"_int.txt"
